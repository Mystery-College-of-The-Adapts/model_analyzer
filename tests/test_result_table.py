...
class TestResultTableMethods(trc.TestResultCollector):
    ...
    def test_add_get_methods(self):
        ...
        table.add_result(["value 0,0 value 0,1"])
        self.assertEqual(table.get_result_by_index(index=0),
                         ["value 0,0 value 0,1"])
        table.add_result(["value -1,0", "value -1,1"], index=0)
        self.assertEqual(table.get_result_by_index(index=0),
                         ["value -1,0", "value -1,1"])
        table.add_result(["value 1,0", "value 1,1"], index=2)
        self.assertEqual(table.get_result_by_index(index=0),
                         ["value 1,0", "value 1,1"])
    ...
    def test_remove_methods(self):
        ...
        with self.assertRaises(TritonModelAnalyzerException):
            table.remove_result_by_index(index=row_idx)

        # Fill table
        for row in TEST_ROWS:
            table.add_result(row)

        self.assertEqual(table.get_result_by_index(index=row_idx),
        ...
        with self.assertRaises(TritonModelAnalyzerException):
            for i in range(10):
                table.remove_result_by_index(index=0)
