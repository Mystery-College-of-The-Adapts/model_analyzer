...
class MetricsManager:
    ...
    def __init__(self, config, metric_tags, server, result_manager):
        ...
        self._configure_result_manager(priorities=priorities)
    ...
    def _categorize_metrics(self, metrics):
        ...
        for metric in metrics:
            if metric in DCGMMonitor.model_analyzer_to_dcgm_field:
                ...
                self._non_dcgm_metrics.append(metric)
            elif metric in CPUMonitor.cpu_metrics:
                self._cpu_metrics.append(metric)
                self._non_dcgm_metrics.append(metric)

    def _configure_result_manager(self, priorities):
        """
        ...
        self._result_manager.create_tables(
            ...
            non_gpu_specific_metrics=self._non_dcgm_metrics,
            aggregation_tag='Max')

        self._result_manager.set_result_comparator(
            ResultComparator(gpu_metric_types=self._dcgm_metrics,
                             non_gpu_metric_types=self._non_dcgm_metrics,
    ...
    def profile_server(self, default_value):
        ...
        self._result_manager.add_server_data(measurements=server_gpu_metrics,
    ...
    def profile_model(self, perf_config, perf_output_writer=None):
        ...
        # Get metrics for model inference
        model_gpu_metrics = self._get_gpu_inference_metrics()
        model_cpu_metrics = self._get_cpu_inference_metrics()

        self._result_manager.add_model_data(measurements=model_gpu_metrics,
                                            has_gpu_ids=True)

        # Combine metrics that do not have GPU ID
        ...
        self._result_manager.add_model_data(
            measurements=model_non_gpu_metric_values, has_gpu_ids=False)
